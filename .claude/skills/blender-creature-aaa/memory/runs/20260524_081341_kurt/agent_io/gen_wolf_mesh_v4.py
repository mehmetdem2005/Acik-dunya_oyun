...
# muzzle: uzun ince taper (kopru biraz asagi)
for i,(yy,zz,rr) in enumerate([(0.655,0.775,0.046),(0.70,0.765,0.040),(0.745,0.756,0.035),
                                (0.79,0.748,0.030),(0.83,0.741,0.026),(0.865,0.735,0.022)]):
    ball((0,yy,zz),rr,stiff=3.0)
ball((0,0.885,0.730),0.018)                     # nose pad
# alt cene/dudak hatti (muzzle altinda ince)
for yy,zz,rr in [(0.70,0.722,0.030),(0.755,0.716,0.026),(0.81,0.712,0.022),(0.855,0.712,0.018)]:
    ball((0,yy,zz),rr,stiff=3.0)
# ===== BACAKLAR =====
...
for i in range(12):
    nm=f"tail_{i}"
    if nm in bones: seg(nm, max(0.072-0.0038*i,0.022), max(0.072-0.0038*(i+1),0.020))
