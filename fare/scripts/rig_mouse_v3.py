...
def build_armature(A: Anatomy):
    ...
    # ---- Cheeks (L/R) — independent expression bones ----
    for side, sx in (("L", -1), ("R", +1)):
        # Cheek = side of head, between skull_center and snout, lateral
    ...
    # ---- Nose tip — the signature mouse twitch ----
    # Tiny bone at the very tip of the snout. Heads at base of snout, tail at nose tip.
    nose_base = A.skull_center.lerp(A.nose_tip, 0.65)
    mk("DEF-nose_tip", nose_base, A.nose_tip,
        parent=head_bone, coll="deform")
    mk("CTRL-nose_tip", nose_base, A.nose_tip,
        parent=head_ctrl, role="CTRL", coll="ctrl_face")
    ...
    # ---- Belly bone — under chest, breathing-driven belly rise ----
    belly_head = V(cx, A.p_chest.y - d * A.y_size * 0.05, A.p_chest.z - A.z_size * 0.15)
    belly_head = _snap_to_inside(A.bvh, belly_head)
    belly_tail = V(cx, A.p_chest.y, A.p_chest.z - A.z_size * 0.05)
    mk("DEF-belly", belly_head, belly_tail,
        parent=spine_bones["DEF-chest"], coll="deform", bbones=2)

    # ---- Ribcage: 4 pairs of ribs, anchored to spine_01/spine_02 ----
    # Ribs span from spine to lateral chest, allowing breath-driven expansion.
    rib_count = 4
    for ri in range(rib_count):
        # Ribs distributed along the chest span (from spine_02 toward chest)
        t = (ri + 1) / (rib_count + 1)
        spine_pt = A.p_spine_2.lerp(A.p_chest, t)
        # Anchor on spine, parent = appropriate spine bone
        spine_parent = spine_bones["DEF-spine_02"] if t < 0.5 else spine_bones["DEF-chest"]
        for side, sx in (("L", -1), ("R", +1)):
            # Rib goes from spine point laterally outward and slightly downward
            rib_head = spine_pt
            rib_tail = V(cx + sx * A.x_half * 0.70,
                          spine_pt.y,
                          spine_pt.z - A.z_size * 0.15)
            # Make sure rib_tail is inside or near the mesh
            rib_tail = _snap_to_inside(A.bvh, rib_tail)
            mk(f"DEF-rib_{ri+1:02d}.{side}", rib_head, rib_tail,
                parent=spine_parent, coll="deform", bbones=2)
