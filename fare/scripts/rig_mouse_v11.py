...
Usage:
    blender --background --python fare/scripts/rig_mouse_v11.py
"""
...
OUT_BLEND = os.path.join(OUT_DIR, "stage3_rig_v11.blend")
os.makedirs(OUT_DIR, exist_ok=True)


def log(m): print(f"[rig11] {m}", flush=True)
...
def build_rig(tripo, mesh_obj):
    ...
    arm_obj.name = "MouseRig_v11"
    arm = arm_obj.data
    arm.name = "MouseRigData_v11"
    eb = arm.edit_bones
    ...
    for side in ("L", "R"):
        ...
        log(f"  front_{side}: shoulder={tuple(round(c,3) for c in sh_h_tr)} "
            f"actual_paw={tuple(round(c,3) for c in actual_paw_pos)}")

        # Scapula at shoulder area, parented to thoracic_01 (anatomical fix from v10)
        scap_parent = "thoracic_01" if "thoracic_01" in created else cog_b
        scap_h = snap_to_inside(bvh, sh_h_tr)
        scap_t = snap_to_inside(bvh, sh_t_tr)
        scap_b = mk(f"scapula_{side}", scap_h, scap_t, parent=scap_parent, bbones=2)

        # Build STRAIGHT chain: shoulder → elbow → wrist → paw → toes (all heading DOWN)
        shoulder_pos = scap_t
        paw_pos = Vector((actual_paw_pos.x, actual_paw_pos.y,
                          max(z_min + 0.002, actual_paw_pos.z)))
        elbow_pos = Vector(
            (shoulder_pos.x * 0.5 + paw_pos.x * 0.5,
             shoulder_pos.y * 0.7 + paw_pos.y * 0.3,  # elbow bends backward slightly
             shoulder_pos.z * 0.4 + paw_pos.z * 0.6))
        elbow_pos = snap_to_inside(bvh, elbow_pos)
        wrist_pos = Vector(
            (elbow_pos.x * 0.3 + paw_pos.x * 0.7,
             elbow_pos.y * 0.3 + paw_pos.y * 0.7,
             elbow_pos.z * 0.3 + paw_pos.z * 0.7))
        wrist_pos = snap_to_inside(bvh, wrist_pos)

        upper_b = mk(f"upper_front_leg_{side}", shoulder_pos, elbow_pos,
                      parent=scap_b, connect=True, bbones=2)
        lower_b = mk(f"lower_front_leg_{side}", elbow_pos, wrist_pos,
                      parent=upper_b, connect=True, bbones=2)
        paw_b = mk(f"front_paw_{side}", wrist_pos, paw_pos,
    ...
    for side in ("L", "R"):
        sx = +1 if side == "L" else -1
        # Use Tripo's hip position as the hip socket anchor (reasonable)
        hip_name = "bone_27" if side == "L" else "bone_17"
        hip_h_tr, hip_t_tr, _ = tripo[hip_name]
        # Detect actual mesh paw position
        actual_paw_pos = detect_back_paw_pos(side)
        if actual_paw_pos is None:
            actual_paw_pos = Vector((hip_t_tr.x, hip_t_tr.y * 0.5, z_min + 0.005))
        log(f"  back_{side}: hip={tuple(round(c,3) for c in hip_h_tr)} "
            f"actual_paw={tuple(round(c,3) for c in actual_paw_pos)}")

        # Build STRAIGHT anatomical chain: hip socket → knee → ankle → paw → toes
        # All going from upper-back DOWN to floor, no zig-zag
        hip_socket = snap_to_inside(bvh, hip_h_tr)
        # Knee position: between hip and paw at ~50% Y but in mid-Z range
        knee_pos = Vector(
            (hip_socket.x * 0.5 + actual_paw_pos.x * 0.5,
             hip_socket.y * 0.7 + actual_paw_pos.y * 0.3,  # bias toward hip (knee bends forward)
             hip_socket.z * 0.4 + actual_paw_pos.z * 0.6))
        knee_pos = snap_to_inside(bvh, knee_pos)
        # Ankle: between knee and paw
        ankle_pos = Vector(
            (knee_pos.x * 0.3 + actual_paw_pos.x * 0.7,
             knee_pos.y * 0.3 + actual_paw_pos.y * 0.7,
             knee_pos.z * 0.3 + actual_paw_pos.z * 0.7))
        ankle_pos = snap_to_inside(bvh, ankle_pos)
        # Paw position: at floor, near actual paw centroid
        paw_pos = Vector((actual_paw_pos.x, actual_paw_pos.y, max(z_min + 0.002, actual_paw_pos.z)))
        # Hip bone: short strut at hip socket
        hip_bone_tail = Vector(
            (hip_socket.x + (knee_pos.x - hip_socket.x) * 0.25,
             hip_socket.y + (knee_pos.y - hip_socket.y) * 0.25,
             hip_socket.z + (knee_pos.z - hip_socket.z) * 0.25))
        hip_bone_tail = snap_to_inside(bvh, hip_bone_tail)

        hip_b = mk(f"hip_{side}", hip_socket, hip_bone_tail,
                    parent=pelvis_b, bbones=2)
        thigh_b = mk(f"thigh_{side}", hip_bone_tail, knee_pos,
                      parent=hip_b, connect=True, bbones=2)
        shin_b = mk(f"shin_{side}", knee_pos, ankle_pos,
                     parent=thigh_b, connect=True, bbones=2)
        paw_b = mk(f"back_paw_{side}", ankle_pos, paw_pos,
                    parent=shin_b, connect=True)
        # 5 toes: spread laterally from paw, all snapped to mesh interior
        for i in range(5):
            spread = (i - 2) * 0.20
            toe_h = paw_pos.copy()
            toe_t = paw_pos + Vector((sx * spread * 0.015,
                                       head_dir * 0.015,
                                       -0.002))  # slight forward + down
            # Toes are at mesh extremity — don't snap (acceptable to be near surface)
            toe_t = Vector((toe_t.x, toe_t.y, max(z_min + 0.0005, toe_t.z)))
