...
import rig_mouse_v2 as rig_module
...
def build_rig():
    ...
    rig1.smooth_weights(mesh, iterations=2)
...
def main():
    ...
    poses = {
        "01_rest":       {},
        # Rotate CTRL bones (intended usage — DEF bones constrained to copy these)
        "02_head_up":    {"CTRL-head": (math.radians(-60), 0, 0)},
        "03_tail_up":    {f"CTRL-tail_{i}": (math.radians(-30), 0, 0) for i in (1, 2, 3)},
        "04_hip_twist":  {"CTRL-hips": (0, 0, math.radians(35))},
        "05_chest_bend": {"CTRL-chest": (math.radians(40), 0, 0)},
        "06_ears_up":    {"CTRL-ear.L": (math.radians(-50), 0, 0),
                          "CTRL-ear.R": (math.radians(-50), 0, 0)},
        "07_jaw_open":   {"CTRL-jaw": (math.radians(45), 0, 0)},
