"""
Stage 3 QA v9 — AAA anatomical mouse rig validator.
...
Usage:
    blender --background --python fare/scripts/stage3_qa_v9.py
"""
...
QA_DIR = os.path.join(ROOT, "out", "qa_stage3_v9")
os.makedirs(QA_DIR, exist_ok=True)
sys.path.insert(0, os.path.join(ROOT, "scripts"))
import rig_mouse_v9 as rig_module
...
def qa_gates(arm, mesh, meta):
    ...
    log(f"=== Stage 3 v9 GATE: {n_pass}/{len(checks)} PASS, {n_fail} FAIL ===")
...
def main():
    ...
    log("")
    log("=" * 60)
    log(f"STAGE 3 v9 GATE: {'PASS ✓' if n_fail == 0 else f'FAIL ({n_fail} issues)'}")
    log("=" * 60)
    log("")
