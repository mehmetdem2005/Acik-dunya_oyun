...
Usage:
    blender --background --python fare/scripts/rig_mouse_v10.py
"""
...
OUT_BLEND = os.path.join(OUT_DIR, "stage3_rig_v10.blend")
os.makedirs(OUT_DIR, exist_ok=True)


def log(m): print(f"[rig10] {m}", flush=True)
...
def build_rig(tripo, mesh_obj):
    ...
    arm_obj.name = "MouseRig_v10"
    arm = arm_obj.data
    arm.name = "MouseRigData_v10"
    eb = arm.edit_bones
    ...
    for side, sh_name, limb_prefix in (("L", "bone_9", "tripo::0_Left_Limb"),
        ...
        lower_h, lower_t, _ = tripo[f"{limb_prefix}_1"]
        paw_h, paw_t, _ = tripo[f"{limb_prefix}_2"]
        # Anatomical fix: scapula attaches to upper thoracic spine (chest), not COG.
        # In real mammals the scapula slides on the ribcage at the shoulder area.
