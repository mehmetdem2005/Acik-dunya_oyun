...
def skin_with_corrective_smooth(mesh_obj, arm_obj):
    ...
    # Add CORRECTIVE_SMOOTH but at factor=0 by default — only enable as final
    # POLISH after weights pass the QA gate. (ChatGPT/pipeline rule: do not use
    # corrective smooth to HIDE bad weights.)
    cs = mesh_obj.modifiers.new(name="CorrectiveSmooth", type='CORRECTIVE_SMOOTH')
    cs.smooth_type = 'LENGTH_WEIGHTED'
    cs.factor = 0.0   # disabled — bare weights must pass first
    cs.iterations = 3
    cs.use_only_smooth = False
    cs.rest_source = 'ORCO'
    log(f"  CorrectiveSmooth added: factor=0.0 (disabled until weights pass QA)")
