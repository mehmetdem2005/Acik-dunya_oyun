"""
AAA mouse rigger v3 — full anatomical detail (ribcage, cheeks, nose tip,
tongue, eyelids, detailed spine, 12-segment tail).

v2 had a working medial-axis spine but was still "pipe-like" — no ribs, no
cheeks, no nose tip independence, short tail, etc. v3 adds layered anatomy:

  - **Ribcage**: 4 pairs of rib bones, breathing-driven (chest expands laterally)
  - **Cheek L/R**: independent expression bones
  - **Nose tip**: dedicated bone for the signature mouse nose-twitch
  - **Tongue**: 2-seg chain for vocal animations
  - **Eyelid upper/lower L/R**: for blink animation
  - **Detailed spine**: 6 vertebra segments (was 4), bbones=3 each
  - **Detailed tail**: 12 segments (was 8), bbones=4 (super smooth)
  - **Belly bone**: under chest, breathing-driven belly rise
  - **Improved jaw weights**: jaw restricted to lower mouth only
  - **Improved ear weights**: ears get own influence zone

Usage:
    blender --background --python fare/scripts/rig_mouse_v3.py
...
def log(m): print(f"[rig3] {m}", flush=True)
...
def build_armature(A: Anatomy):
    ...
    # HEAD chain split into 3 segments per Rigging Anatomy agent's fix #1:
    #   DEF-head   : back_of_skull → front_of_skull  (cranium)
    #   DEF-muzzle  : front_of_skull → snout_base     (flex muzzle)
    #   DEF-nose_tip: snout_base → nose_tip           (twitch)
    # This kills the "pipe head" — muzzle bends independently from skull.
    front_of_skull = A.skull_center.lerp(A.nose_tip, 0.30)
    snout_base = A.skull_center.lerp(A.nose_tip, 0.65)

    head_bone = mk("DEF-head", back_of_skull, front_of_skull,
                     parent=neck_bone, connect=True, coll="deform", bbones=2)
    muzzle_bone = mk("DEF-muzzle", front_of_skull, snout_base,
                      parent=head_bone, connect=True, coll="deform", bbones=2)
    spine_bones["DEF-head"]   = head_bone
    spine_bones["DEF-muzzle"] = muzzle_bone

    # JAW bone: lower mandible — restricted to lower jaw cavity only
    jaw_head = A.p_jaw
    jaw_tail = A.nose_tip
    if (jaw_tail - jaw_head).length < 1e-3:
        jaw_tail = jaw_head + V(0, d * 0.04, 0)
    # Jaw parents to SKULL (not muzzle), so when muzzle flexes the jaw stays
    # with the cranium (correct anatomy — jaw hinges at the skull, not the snout).
    jaw_bone = mk("DEF-jaw", jaw_head, jaw_tail, parent=head_bone, coll="deform")

    # Spine CTRL mirrors (CTRL-head = whole head chain)
    ...
    head_ctrl  = mk("CTRL-head",  back_of_skull, front_of_skull, parent=chest_ctrl, role="CTRL", coll="ctrl_face")
    muzzle_ctrl = mk("CTRL-muzzle", front_of_skull, snout_base, parent=head_ctrl, role="CTRL", coll="ctrl_face")
    ...
    # ---- PELVIS bone (Rigging Anatomy fix #2): between root and hips. ----
    # Back thighs parent to PELVIS, not directly to spine — proper hip socket.
    pelvis_head = A.p_hips
    pelvis_tail = A.p_spine_1
    pelvis_bone = mk("DEF-pelvis", pelvis_head, pelvis_tail,
                      parent=spine_bones["DEF-hips"], connect=False, coll="deform")

    # ---- SCAPULA bones (L/R) — floating shoulder blade ----
    # Quadrupeds need this for shrug/locomotion. Parent: chest. Children: arm bones.
    scapula_bones = {}
    for side, sx in (("L", -1), ("R", +1)):
        # Scapula head at the top-back of the shoulder blade, on the rib cage.
        scap_head = V(cx + sx * A.x_half * 0.18,
                       A.p_shoulders.y - d * A.y_size * 0.02,
                       A.p_shoulders.z + A.z_size * 0.05)
        scap_head = _snap_to_inside(A.bvh, scap_head)
        # Scapula tail toward shoulder joint (where humerus attaches)
        scap_tail = V(cx + sx * A.x_half * 0.30,
                       A.p_shoulders.y + d * A.y_size * 0.02,
                       A.p_shoulders.z - A.z_size * 0.05)
        sb = mk(f"DEF-scapula.{side}", scap_head, scap_tail,
                 parent=spine_bones["DEF-chest"], coll="deform")
        scapula_bones[side] = sb
    ...
    for side, sx in (("L", -1), ("R", +1)):
        ...
        for i in range(5):
            ...
            path.append(p)
        # SHOULDER (clavicle) now parents to SCAPULA, not chest directly
        sh = mk(f"DEF-shoulder.{side}", path[0], path[1], parent=scapula_bones[side], coll="deform")
    ...
    for side, sx in (("L", -1), ("R", +1)):
        ...
        for i in range(5):
            ...
            p = _snap_to_inside(A.bvh, p) if i < 4 else p
            path4.append(p)
        # THIGH parents to PELVIS (proper hip socket), not directly to hips spine
        th = mk(f"DEF-thigh.{side}", path4[0], path4[1], parent=pelvis_bone, coll="deform")
    ...
    # ---- Ears (parent to SKULL — anatomically correct, ears are on cranium) ----
    ...
    # ---- Whiskers (parent to MUZZLE — whiskers fan from the snout, not skull) ----
    # When muzzle flexes, whiskers follow naturally. AAA agent's note #2 fix.
    for side, sx in (("L", -1), ("R", +1)):
        for i in range(4):
            t = i / 3.0
            ...
            mk(f"DEF-whisker_{i+1:02d}.{side}", base, tip,
                parent=muzzle_bone, coll="deform", bbones=4)

    # ---- Eyes (parent to SKULL — eyes sit in cranial orbits) ----
    ...
    for side in ("L", "R"):
        ...
        # ---- Eyelids (parent to SKULL) ----
        sx_lid = -1 if side == "L" else +1
        for which, dz in (("upper", +0.012), ("lower", -0.012)):
            lid_head = eye_pos_fixed
            lid_tail = eye_pos_fixed + V(sx_lid * 0.01, d * 0.01, dz)
            mk(f"DEF-eyelid_{which}.{side}", lid_head, lid_tail,
                parent=head_bone, coll="deform")
            mk(f"CTRL-eyelid_{which}.{side}", lid_head, lid_tail,
                parent=head_ctrl, role="CTRL", coll="ctrl_face")

    # ---- Cheeks (parent to SKULL but extend toward muzzle area) ----
    for side, sx in (("L", -1), ("R", +1)):
        cheek_y = A.skull_center.y + d * A.y_size * 0.05
        cheek_z = A.skull_center.z - A.z_size * 0.05
        cheek_head = V(cx + sx * A.x_half * 0.20, cheek_y, cheek_z)
        cheek_head = _snap_to_inside(A.bvh, cheek_head)
        cheek_tail = V(cx + sx * A.x_half * 0.55, cheek_y + d * A.y_size * 0.04, cheek_z + A.z_size * 0.02)
        mk(f"DEF-cheek.{side}", cheek_head, cheek_tail,
            parent=head_bone, coll="deform", bbones=2)
        mk(f"CTRL-cheek.{side}", cheek_head, cheek_tail,
            parent=head_ctrl, role="CTRL", coll="ctrl_face")

    # ---- Nose tip (parent to MUZZLE — twitch follows muzzle flex) ----
    mk("DEF-nose_tip", snout_base, A.nose_tip,
        parent=muzzle_bone, coll="deform")
    mk("CTRL-nose_tip", snout_base, A.nose_tip,
        parent=muzzle_ctrl, role="CTRL", coll="ctrl_face")

    # ---- NOSTRILS (2 — independent of nose_tip) ----
    # AAA agent fix: real mouse has flaring nostrils.
    for side, sx in (("L", -1), ("R", +1)):
        nostril_pos = V(cx + sx * A.x_half * 0.15, A.nose_tip.y - d * 0.01, A.nose_tip.z + 0.005)
        mk(f"DEF-nostril.{side}", nostril_pos, nostril_pos + V(sx * 0.005, d * 0.005, 0.005),
            parent=muzzle_bone, coll="deform")

    # ---- LIPS (upper, lower, corner L/R = 4 lip bones) ----
    # AAA agent fix #4: lips needed for proper face read.
    lip_base_y = A.p_jaw.y + d * A.y_size * 0.03
    lip_z = A.p_jaw.z + A.z_size * 0.02
    mk("DEF-lip_upper", V(cx, lip_base_y, lip_z + 0.015),
        V(cx, lip_base_y + d * 0.015, lip_z + 0.015),
        parent=muzzle_bone, coll="deform")
    mk("DEF-lip_lower", V(cx, lip_base_y, lip_z - 0.005),
        V(cx, lip_base_y + d * 0.015, lip_z - 0.005),
        parent=jaw_bone, coll="deform")
    for side, sx in (("L", -1), ("R", +1)):
        corner_pos = V(cx + sx * A.x_half * 0.20, A.p_jaw.y, A.p_jaw.z + 0.01)
        mk(f"DEF-lip_corner.{side}", corner_pos, corner_pos + V(sx * 0.01, d * 0.01, 0),
            parent=head_bone, coll="deform")
    # CTRL versions
    mk("CTRL-lip_upper", V(cx, lip_base_y, lip_z + 0.015),
        V(cx, lip_base_y + d * 0.015, lip_z + 0.015),
        parent=muzzle_ctrl, role="CTRL", coll="ctrl_face")
    mk("CTRL-lip_lower", V(cx, lip_base_y, lip_z - 0.005),
        V(cx, lip_base_y + d * 0.015, lip_z - 0.005),
        parent=jaw_ctrl, role="CTRL", coll="ctrl_face")

    # ---- Tongue (2 segments) ----
    # Inside the lower jaw area
    tongue_base = A.p_jaw + V(0, d * A.y_size * 0.01, A.z_size * 0.01)
    tongue_mid = A.p_jaw + V(0, d * A.y_size * 0.04, A.z_size * 0.01)
    tongue_tip = A.p_jaw + V(0, d * A.y_size * 0.07, A.z_size * 0.01)
    t1 = mk("DEF-tongue_01", tongue_base, tongue_mid, parent=jaw_bone, coll="deform")
    t2 = mk("DEF-tongue_02", tongue_mid, tongue_tip, parent=t1, connect=True, coll="deform")

    # ---- BELLY (longer chain, 2 segments — Rigging Anatomy fix) ----
    # A single belly stick can't deform from sternum to pelvis; use 2 segments.
    belly_top = V(cx, A.p_chest.y - d * A.y_size * 0.02, A.p_chest.z - A.z_size * 0.18)
    belly_mid = V(cx, A.p_spine_2.y, A.p_spine_2.z - A.z_size * 0.18)
    belly_bot = V(cx, A.p_hips.y + d * A.y_size * 0.02, A.p_hips.z - A.z_size * 0.18)
    belly_top = _snap_to_inside(A.bvh, belly_top)
    belly_mid = _snap_to_inside(A.bvh, belly_mid)
    belly_bot = _snap_to_inside(A.bvh, belly_bot)
    belly_01 = mk("DEF-belly_01", belly_top, belly_mid,
                   parent=spine_bones["DEF-chest"], coll="deform", bbones=2)
    belly_02 = mk("DEF-belly_02", belly_mid, belly_bot,
                   parent=belly_01, connect=True, coll="deform", bbones=2)

    # ---- STERNUM (Rigging Anatomy fix #3) — ventral midline, anchors ribs ----
    sternum_top = V(cx, A.p_chest.y + d * A.y_size * 0.02, A.p_chest.z - A.z_size * 0.12)
    sternum_top = _snap_to_inside(A.bvh, sternum_top)
    sternum_bot = V(cx, A.p_spine_2.y - d * A.y_size * 0.02, A.p_spine_2.z - A.z_size * 0.14)
    sternum_bot = _snap_to_inside(A.bvh, sternum_bot)
    sternum_bone = mk("DEF-sternum", sternum_top, sternum_bot,
                       parent=spine_bones["DEF-chest"], coll="deform", bbones=2)

    # ---- RIBCAGE (curved arcs, Rigging Anatomy fix #3) ----
    # 4 pairs of ribs, each rendered with bbones=3 for a curve. The rib
    # sweeps ventrally and caudally (down AND back) from spine to sternum.
    rib_count = 4
    for ri in range(rib_count):
        t = (ri + 1) / (rib_count + 1)
        spine_pt = A.p_spine_2.lerp(A.p_chest, t)
        sternum_pt = sternum_top.lerp(sternum_bot, t)
        spine_parent = spine_bones["DEF-spine_02"] if t < 0.5 else spine_bones["DEF-chest"]
        for side, sx in (("L", -1), ("R", +1)):
            rib_head = spine_pt
            # Rib tail sweeps to a point near the sternum on the same side
            rib_tail = V(cx + sx * A.x_half * 0.18,
                          sternum_pt.y,
                          sternum_pt.z + A.z_size * 0.02)
            rib_tail = _snap_to_inside(A.bvh, rib_tail)
            # bbone_segments=3 gives a curving rib
            mk(f"DEF-rib_{ri+1:02d}.{side}", rib_head, rib_tail,
                parent=spine_parent, coll="deform", bbones=3)

    # ---- Brow (forehead) bones for expression — L/R ----
    for side, sx in (("L", -1), ("R", +1)):
        brow_head = V(cx + sx * A.x_half * 0.25,
                       A.skull_center.y - d * A.y_size * 0.02,
                       A.skull_center.z + A.z_size * 0.05)
        brow_head = _snap_to_inside(A.bvh, brow_head)
        brow_tail = V(cx + sx * A.x_half * 0.50,
                       A.skull_center.y + d * A.y_size * 0.04,
                       A.skull_center.z + A.z_size * 0.08)
        mk(f"DEF-brow.{side}", brow_head, brow_tail,
            parent=head_bone, coll="deform")
        mk(f"CTRL-brow.{side}", brow_head, brow_tail,
            parent=head_ctrl, role="CTRL", coll="ctrl_face")
...
def main():
    ...
    # extra fields the v1 manual_weight_overrides expects
    A_shim.x_center = A.x_center
    A_shim.head_dir = A.head_dir
    A_shim.x_min = A.x_min; A_shim.x_max = A.x_max
    A_shim.y_min = A.y_min; A_shim.y_max = A.y_max
    A_shim.z_min = A.z_min; A_shim.z_max = A.z_max
    A_shim.verts = A.verts
    A_shim.mesh = A.mesh
    A_shim.y_head = A.skull_center.y
    A_shim.y_nose = A.nose_tip.y
    A_shim.y_neck = A.p_neck.y
    A_shim.y_jaw  = A.p_jaw.y
    A_shim.z_eyes = A.skull_center.z
    A_shim.z_head = A.skull_center.z
    A_shim.z_floor = A.z_floor
    A_shim.z_belly = A.z_belly
    ...
    # Re-enable manual weight overrides (Skinning Agent fix #1, #2):
    #   isolate ear/jaw/tail vertices to their own DEF bones at weight=1.0 with
    #   clear=[neighbor bones]. Otherwise heat-weighted neighbors hold ~0.45+
    #   on those verts and the dedicated bone can't deform them.
    v1.manual_weight_overrides(mesh_obj, A_shim)
    # Skinning Agent fix #3: per-region smoothing (gentler) instead of global blur
    v1.smooth_weights(mesh_obj, iterations=1)
